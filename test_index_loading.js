...
function generateBlocks(num = 10) {
  ...
  for (let i = 0; i < num; i += 1) {
    ...
    else value= prompt("enter values");
...
function Pseudo(opt) //0 for bubble and 1 for insertion
{
  ...
  if(opt==-1)
  var code=["l=0","r=length(array)-1","while(l<=r):","&nbsp;&nbsp;mid=(l+r)//2","&nbsp;&nbsp;if(array[mid]==x)","&nbsp;&nbsp;&nbsp;&nbsp;return mid","&nbsp;&nbsp;else if(array[mid]<x)","&nbsp;&nbsp;&nbsp;&nbsp;l=mid+1","&nbsp;&nbsp;else","&nbsp;&nbsp;&nbsp;&nbsp;r=mid-1"];
  if(opt===3)
      ...
      for (let i =index;i<=index;i += 1){
         if(pseudos[i]!=-1) {
          ...
          if(i<pseudos.length && pseudos[i+1]!=-1){
            if(pseudos[i+1]==0){
              pseudo[2].style.color="black";
            }
            pseudo[pseudos[i+1]+1].style.color="black";
          }
        }
        else{
            if(i<pseudos.length && pseudos[i+1]!=-1){
    ...
    //  await sleep(500);
     ...
     appender([0,blocks[i]],"Is the element highlighted equal to the value?",1);
      if (value1 == value2) {
        ...
        appender([1,blocks[i]],"Yes! the element highlighted is equal to the value",2);
        blocks[i].style.backgroundColor="#13CE66";
        console.log(ele);
      }
      else{
...
async function fwdb(){
    ...
    for (let i =states.length-1;i>=0;i -= 1){
        let x=states[i][0];
        if(x==0){
            blocks[states[i][1]].style.backgroundColor="#58b7ff";
        }
        else if(x==1){
            blocks[states[i][1]].style.backgroundColor="green";
        }
        else if(x==2){
            for(let k=0;k<=states[i][1];k+=1)
              blocks[k].style.backgroundColor="#58b7ff";
        }
        else if(x==3){
            for(let k=states[i][1];k<blocks.length;k+=1)
              blocks[k].style.backgroundColor="#58b7ff";
        }
        else {
            blocks[states[i][1]].style.backgroundColor="#ff4949";
        }
    }
}
...
{ generateBlocks();
  ...
  setTimeout(function() { x=prompt("Enter the value, to be searched"); }, 10);
    while(pause){
      await sleep(1);
    }
  ...
  {     while(pause){
    ...
    appender([4,mid],"done with this mid");
  }   while(pause){
      await sleep(1);
    }
    fwdb();
